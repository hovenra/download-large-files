...
public class TransactionService {
    ...
    private static Transaction mapToTransaction(String line) throws ArrayIndexOutOfBoundsException{
        final String[] p = line.split(",");
        return new Transaction(p[0], p[1], p[2], p[3], LocalDateTime.now(), p[4], p[5], p[6], p[7], p[8], p[9], p[10], p[11], p[12], p[13]);
    }

    public Flowable<Transaction> getConnectableTransactions() {
        return ConnectableFlowable.create(new FlowableOnSubscribe<Transaction>() {
            @Override
